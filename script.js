...
$(document).ready(function () {
    ...
    var myKey = "f299017fc79bef68bd06401cc604c72e";
    ...
    function weatherNow(event) {
        ...
        $.ajax({
            ...
            var city = $("<h1>").addClass("city-name").text(`City: ${response.name}`);
            var date = $("<h3>").addClass("date").text(`Date: ${time}`);
            var iconImage = $("<img>").addClass("icon-image").attr("src", "https://openweathermap.org/img/w/" + response.weather[0].icon + ".png");
            var tempF = parseInt((response.main.temp - 273.15) * 1.8 + 32); //kelvin to farenheight Conversion
            var temperature = $("<h4>").addClass("current-temp").text(`Current Temperature: ${tempF} F°`);
            var humidity = $("<h4>").addClass("humidity").text(`Humidity: ${response.main.humidity}%`);
            var windSpeed = $("<h4>").addClass("wind-speed").text(`Wind Speed ${response.wind.speed} mph`);
